...
import { FewFromListTypes } from 'src/components/organisms/CardCreation/CardVerificationQuestion/types';

function FewFromList(props: FewFromListTypes.IProps) {
  const { list, onListChange, errors, onErrorsChange, onNewOptionAdd, onOptionDelete } = props;
  const { isIncludedInBase, isQuizCard } = useContext(CardCreationContext);

  const { timeToAnswer, instruction, question, appendix } = list;

  useEffect(() => {
    isQuizCard && onListChange({ ...list, answerOptions: { ...list.answerOptions, isShuffled: true } });
  },        []);
